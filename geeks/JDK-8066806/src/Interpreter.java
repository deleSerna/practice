...
public final class Interpreter {
    ...
    static class CS extends MutableCallSite {
        ...
        public CS(String debug) {
            super(MethodType.methodType(void.class, Interpreter.class, int.class));
            this.debug = debug;
        ...
        static {
            ...
            try {
                ...
                TEST = lookup.findStatic(CS.class, "test", MethodType.methodType(boolean.class, Interpreter.class, int.class, int.class));
            } catch (NoSuchMethodException | IllegalAccessException e) {
                throw new AssertionError(e);
            }
            Ops[] ops = Ops.values();
            MethodHandle[] table = new MethodHandle[ops.length];
            Arrays.setAll(table, i -> snippet(ops[i]));
            INSTR_TABLE = table;
        }
    }

    static MethodHandle snippet(Ops ops) {
        try {
            return lookup().findVirtual(Interpreter.class, ops.name(), methodType(void.class));
        } catch (NoSuchMethodException | IllegalAccessException e) {
            throw new AssertionError(e);
        }
    }
